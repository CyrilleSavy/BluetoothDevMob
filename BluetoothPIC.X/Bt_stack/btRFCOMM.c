...
static void btRfcommTxData(RfcommInstanceState* state, uint8_t dlci, sg_buf* buf){
    ...
    if(state->ports[dlci].creditsOwed){ //use the oportunity to send the credits
        SIOPrintString("RFCOMM Send credits\r\n");
    ...
    if(sg_add_front(buf, data, hdrLen, SG_FLAG_MAKE_A_COPY)){
        ...
        if(sg_add_back(buf, data, 1, SG_FLAG_MAKE_A_COPY)){
            SIOPrintString("RFCOMM Packet sended\r\n");
            l2capServiceTx(state->aclConn, state->remChan, buf);
            return;
        }
    }
    SIOPrintString("RFCOMM Send Alloc error\r\n");
    sg_free(buf);
    free(buf);
}
...
static void rfcommServiceDataRx(void* service, const uint8_t* req, uint16_t reqSz){
    ...
    SIOPrintString("RFCOMM Data Rx : DLCI = ");
    SIOPutDec(DLCI);
    SIOPrintString(" ctrl = ");
    SIOPutHex(ctrl);
    SIOPrintString("\r\n");
    ...
    if(ctrl == BT_RFCOMM_CMD_SABM || ctrl == BT_RFCOMM_CMD_DISC){
        ...
        state->ports[DLCI].queued = 0x00;

        if(ctrl == BT_RFCOMM_CMD_SABM){

            if(gPortHandlers[DLCI].oF) 
            {
                gPortHandlers[DLCI].oF(state, DLCI);
            }
            SIOPrintString("DLCI ");
            SIOPutDec(DLCI);
            SIOPrintString(" opened\r\n");
    ...
    if(ctrl == BT_RFCOMM_CMD_UIH_CRED){
        ...
        SIOPrintString("RFCOMM credits : rcv = ");
        SIOPutDec(*(data-1));
        SIOPrintString("\r\n");
        ...
        if(state->ports[DLCI].queued && state->ports[DLCI].credits){
            SIOPrintString("RFCOMM send queued data\r\n");
    ...
    if(ctrl == BT_RFCOMM_CMD_UIH && DLCI != 0){ //we got a msg - send back credits if we already owe the limit, else buffer it

        if(++(state->ports[DLCI].creditsOwed) == BT_RFCOMM_MAX_CRED_DEBT){
            SIOPrintString("RFCOMM Test credits Owe\r\n");
            buf[ 0] = p->addr;
    ...
    if(DLCI == 0){ //control channel transaction
        SIOPrintString("RFCOMM channel transaction\r\n");
        if(ctrl == BT_RFCOMM_CMD_UIH){ //data recieved
            ...
            switch(data[0]){
                ...
                case BT_RFCOMM_PN_C:{		//negotiations are for wusses!
                    ...
                    SIOPrintString("RFCOMM Negociation : Target DLCI = ");
                    SIOPutDec(targetDLCI);
                    SIOPrintString(" Credits = ");
                    SIOPutDec(data[9]);
                    SIOPrintString(" MTU = ");
                    SIOPutDec((((uint16_t)data[7]) << 8) | data[6]);
                    SIOPrintString("\r\n");
                ...
                case BT_RFCOMM_MSC_C:{		//we need to echo back the control signals
                    SIOPrintString("RFCOMM Control signals echo \r\n");
                    if(len > 127) 
                    {
                        SIOPrintString("Control packet way too big\n");
                    }
                    else{
                ...
                case BT_RFCOMM_MSC_R:{
                    SIOPrintString("RFCOMM Nothing to do\r\n");
                ...
                default:
                    ...
                    SIOPrintString("Recieved unhandled RFCOMM packet:");
                    for(i = 0; i < reqSz; i++)
                    {
                        SIOPutHex(req[i]);
                        SIOPutChar(' ');
        ...
        if(len){
            ...
            if(gPortHandlers[DLCI].rF)
            {
                gPortHandlers[DLCI].rF(state, DLCI, data, len);
            }
            SIOPrintString("RFCOMM DLCI ");
            SIOPutDec( p->addr >> 2);
            SIOPrintString(" data:");
            for(i = 0; i < len; i++)
            {
        ...
        SIOPrintString("Recieved unhandled RFCOMM cmd (dlci=%d):", DLCI);
        for(i = 0; i < reqSz; i++) 
        {
            SIOPutHex(req[i]);
            SIOPutChar(' ');
...
void btRfcommRegisterL2capService(){
    ...
    if(!l2capServiceRegister(L2CAP_PSM_RFCOMM, &rfcom))
    {
...
void btRfcommPortTx(void* port, uint8_t dlci, const uint8_t* data, uint16_t size){
    ...
    if(!sg_add_front(buf, data, size, SG_FLAG_MAKE_A_COPY)){
        SIOPrintString("RFCOMM : Error Alloc TX\r\n");
        sg_free(buf);
        free(buf);
    }
    else{
        ...
        if(state->ports[dlci].credits){	//send immediately
            state->ports[dlci].credits--;
            SIOPrintString("RFCOMM credits :");
            SIOPutDec(state->ports[dlci].credits);
            SIOPrintString("\r\n");
