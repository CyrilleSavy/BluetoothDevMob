    ...
    while((*ptr) < end){
        ...
        if(wantedIDs && typ == SDP_TYPE_UUID){
            ...
            if(end < (*ptr)){
                ...
                SIOPrintString("SDP: UUID size > allowed size (%d, %d)\n", sz, end - (*ptr));
                goto out;
            }
            ...
            if(sz > (unsigned)(end - (*ptr))){
                ...
                SIOPrintString("SDP: element size > allowed size (%d, %d)\n", sz, end - (*ptr));
                goto out;
            }
            ...
            if(walkResultP){
                ...
                if(isID){
                    ...
                    if(sz != 2) SIOPrintString("SDP: attrib ID not 16 bits!\n");
...
static sg_buf* btSdpProcessRequest(SdpInstance* inst, const uint8_t* req, uint16_t reqSz){
    ...
    reqSz -= 5;
    if(reqSz != (((uint16_t)req[0]) << 8) + req[1])
    {
        return btSdpError(trans, SDP_ERR_Invalid_PDU_Size);
    }
    req += 2;

    SIOPrintString("SDP request cmd ");
    SIOPutDec(cmd);
    SIOPrintString(" (session ");
    SIOPutHex(trans[0]);
    SIOPutHex(trans[1]);
    SIOPrintString(") with ");
    SIOPutDec(reqSz);
    SIOPrintString(" bytes of data\r\n");


    if(cmd == SDP_PDU_Service_Search_Request || cmd == SDP_PDU_Service_Search_Attribute_Request){

        if((*req) >> 3 != SDP_TYPE_ARRAY) 
        {
            SIOPrintString("Invalid Request : Array \r\n");
        ...
        while(req < end){
            ...
            if(numIDs == MAX_UUIDS_IN_SEARCH) 
            {
                SIOPrintString("Invalid Request : MAX UUID \r\n");
                return btSdpError(trans, SDP_ERR_Invalid_Request_Syntax);	//too many requests
            }
            if(!btSdpGetUUID(&req, &ids[numIDs++]))
            {
                SIOPrintString("Invalid Request : Mal formed UUID\r\n");
                return btSdpError(trans, SDP_ERR_Invalid_Request_Syntax);	//malformed UUID
            }
        }
    }
        ...
        for(i = 0; i < 4; i++) 
        {
            wantedHandle = (wantedHandle << 8) | *req++;
        }
    }
    else{

        SIOPrintString("SDP: invalid request: %d\n", cmd);
        return btSdpError(trans, SDP_ERR_Invalid_Request_Syntax);
    }

    for(i = 0; i < 2; i++) 
    {
        maxReplSz = (maxReplSz << 8) | *req++;
    }
    ...
    if(cmd == SDP_PDU_Service_Attribute_Request || cmd == SDP_PDU_Service_Search_Attribute_Request){
        ...
        if((*req) >> 3 != SDP_TYPE_ARRAY) 
        {
            SIOPrintString("Invalid Request \r\n");
            return btSdpError(trans, SDP_ERR_Invalid_Request_Syntax);
        }
        sz = btSdpGetElemSz(&req);
        end = req + sz;
        ...
        while(req < end){
            ...
            if(numAttrs == MAX_UUIDS_IN_SEARCH)
            {
                SIOPrintString("Invalid Request : Insuff resource \r\n");
                return btSdpError(trans, SDP_ERR_Insufficient_Resources);	//too many -> unsupported request -> fail
            }
            sz = btSdpGetElemSz(&req);
            if(sz == 2){
            ...
            {
                SIOPrintString("Invalid Request : Inv num format \r\n");
                return btSdpError(trans, SDP_ERR_Invalid_Request_Syntax);	//fail -> invalid number format
            }
            attrs[numAttrs++] = sz;
        }
    }
    ...
    if(contStateSz){		// verify continuation is valid or fail
        ...
        if(contStateSz != sizeof(uint32_t))
        {
            SIOPrintString("Invalid Request : Inv cont state \r\n");
            return btSdpError(trans, SDP_ERR_Invalid_Continuation_State);
        }
        for(i = 0; i < 4; i++)
        {
            contState = (contState << 8) | *req++;
        }

        if(contState != inst->contDescr || !inst->result){

            SIOPrintString("SDP: invalid continuation state. Wanted %08X, got %08X\n", inst->contDescr, contState);
            if(inst->result){
        ...
        if(cmd == SDP_PDU_Service_Search_Request || cmd == SDP_PDU_Service_Search_Attribute_Request){
            ...
            for(curSvc = knownServices; curSvc && numFound < MAX_SEARCH_RESULTS; curSvc = curSvc->next){
                ...
                for(num = 0; num < numIDs; num++)
                {
                    uuids_copy[num] = ids[num];
                }

                btStdRecursiveWalk(uuids_copy, &num, NULL, &ptr, curSvc->descrLen);
                if(!num)
                {
                    results[numFound++] = curSvc;
                }
            }
        }
        else if(cmd == SDP_PDU_Service_Attribute_Request){
            ...
            if(!numFound) 
            {
                return btSdpError(trans, SDP_ERR_Invalid_Service_Record_Handle);
            }
        }
        ...
        if(cmd == SDP_PDU_Service_Attribute_Request || cmd == SDP_PDU_Service_Search_Attribute_Request){
            ...
            if(!resultSoFar)
            {
                SIOPrintString("Invalid Request : Insuf res \r\n");
            ...
            for(i = 0; i < numFound; i++){
                ...
                {
                    if(SDP_ATTR_HANDLE >= (attrs[j] >> 16) && SDP_ATTR_HANDLE <= (attrs[j] & 0xFFFF))
                    {
                        break;
                    }
                }
                ...
                if(btSdpPutIntoGroup(res))
                {
                    sg_concat_back(resultSoFar, res);
                }
                sg_free(res);
                free(res);
            }
            ...
            if((cmd == SDP_PDU_Service_Search_Attribute_Request) && !btSdpPutIntoGroup(resultSoFar)){
                SIOPrintString("SDP: Failed to put results into a group\n");
                sg_free(resultSoFar);
                free(resultSoFar);
                return btSdpError(trans, SDP_ERR_Insufficient_Resources);
            }
            ...
            if(!buf){
                ...
                SIOPrintString("SDP: Failed to allocate flattened result array (%ub)\n", sg_length(resultSoFar));
                sg_free(resultSoFar);
                free(resultSoFar);
                return btSdpError(trans, SDP_ERR_Insufficient_Resources);
            }
    ...
    if(++sdpContVal == 0)
    {
        sdpContVal = 0x01234567; //update continuation state to the next value
    }
    ...
    if(cmd == SDP_PDU_Service_Attribute_Request || cmd == SDP_PDU_Service_Search_Attribute_Request){
        ...
        if(maxReplSz > 256)
        {
            maxReplSz = 256;	//no harm in fragmenting - keep the packets small
        }
        ...
        if(maxReplSz > 64)
        {
            maxReplSz = 64;	//no harm in fragmenting - keep the packets small
        }

        sendSz = inst->resultSz;
        if(sendSz > maxReplSz * sizeof(uint32_t))
        {
            sendSz = maxReplSz * sizeof(uint32_t);
        }
    ...
    if(!sg_add_back(result, inst->result, sendSz, SG_FLAG_MAKE_A_COPY)){
        ...
        SIOPrintString("SDP: Failed to attach reply. Droping");
        free(inst->result);
        inst->result = NULL;
        sg_free(result);
        free(result);
    ...
    if(inst->result){ //have more
        ...
        for(i = 0; i < 4; i++)
        {
            bufPostpend[i + 1] = inst->contDescr >> ((3 - i) << 3);
        }
        postSz = 5;
    }
...
//  -- ugly debugging code --
        ...
        SIOPrintString("SDP reply sent (");
         SIOPutHex(sg_length(reply));
        SIOPrintString("):");
        for(i = 0; i < sg_length(reply); i++)
        {
             SIOPutHex(buf[i]);
            SIOPutChar(' ');
        }
...
void btSdpRegisterL2capService(){
    ...
    if(!l2capServiceRegister(L2CAP_PSM_SDP, &sdp))
    {
        SIOPrintString("SDP L2CAP registration failed\n");
    }
...
void btSdpServiceDescriptorAdd(const uint8_t* descriptor, uint16_t descrLen){
    ...
    if(s){
        ...
        {
            sdpNextHandle = SDP_FIRST_USER_HANDLE; //first add is special - it adds the SDP service itself
        }
